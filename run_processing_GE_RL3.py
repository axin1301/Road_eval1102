...
def main(task):
    ...
    if not os.path.exists("folder1102"):
        # 本地保存 ZIP 文件的路径
    ...
    source_folder = "folder1102/GraphSamplingToolkit-main_improve_GE_directly_add_RL3"

    # 指定目标文件夹路径
    destination_folder = "./GraphSamplingToolkit-main_improve_GE_directly_add_RL3"

    # 将整个文件夹移动到目标位置
    if not os.path.exists(destination_folder):
        shutil.move(source_folder, destination_folder)

    py_file_list = ['average_statistics_GE_RL3.py','main_pipeline_GE_RL3.py','main_pipeline_GE_RL3_single_test.py','mapcompare_GE_RL3.py','mapcompare_GE_RL3_single_test.py']
    ...
    if task == 'test':
        python_script = "main_pipeline_GE_RL3_single_test.py"  # 替换为你要运行的 Python 脚本名
        print(f"运行 Python 脚本: {python_script}")
        subprocess.run(["python", python_script], check=True)

    else:
        # 第三步：运行指定的 Python 脚本
        python_script = "main_pipeline_GE_RL3.py"  # 替换为你要运行的 Python 脚本名
        print(f"运行 Python 脚本: {python_script}")
        subprocess.run(["python", python_script], check=True)
