...
def main(task):
    ...
    download_url = "https://anonymous.4open.science/api/repo/Road_eval1102-D498/zip"

    # 本地保存 ZIP 文件的路径
    zip_save_path = "folder1102.zip"
    extract_path = "folder1102"  # 解压到的文件夹
    ...
    source_folder = "folder1102/GraphSamplingToolkit-main_improve_GE_constructed2"

    # 指定目标文件夹路径
    destination_folder = "./GraphSamplingToolkit-main_improve_GE_constructed2"

    # 将整个文件夹移动到目标位置
    if not os.path.exists(destination_folder):
        shutil.move(source_folder, destination_folder)

    py_file_list = ['average_statistics_constructed2.py','main_pipeline_constructed2.py','main_pipeline_constructed2_single_test.py','mapcompare_GE_constructed2.py','mapcompare_GE_constructed2_single_test.py']
    for py_file in py_file_list:
        shutil.copy('folder1102/'+py_file,'code1013/'+py_file)
    ...
    if task == 'test':
        python_script = "main_pipeline_constructed2_single_test.py"  # 替换为你要运行的 Python 脚本名
        print(f"运行 Python 脚本: {python_script}")
        subprocess.run(["python", python_script], check=True)

    else:
        # 第三步：运行指定的 Python 脚本
        python_script = "main_pipeline_constructed2.py"  # 替换为你要运行的 Python 脚本名
        print(f"运行 Python 脚本: {python_script}")
        subprocess.run(["python", python_script], check=True)
